...
object PnLibraryBootstrapInstaller {
    ...
    fun ensureCompatible(plugin: JavaPlugin, minimumVersion: String): Boolean {
        val library = plugin.server.pluginManager.getPlugin("pnLibrary")
        if (library == null || !library.isEnabled) {
            showFailure(plugin, IllegalStateException("pnLibrary не включилась; проверьте её сообщение запуска"))
            return false
        }
        if (isAtLeastVersion(library.description.version, minimumVersion)) return true
        showOutdated(plugin, library.description.version, minimumVersion)
        return false
    }
    ...
    private fun isAtLeastVersion(installed: String, required: String): Boolean =
        BootstrapVersion.isAtLeast(installed, required)

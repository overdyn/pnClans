...
| pnLibrary `2.0.0-beta.3` или новее | Общий runtime, MBox, bStats и зашифрованная диагностика | Да |
...
Декоративный фон GUI настраивается в секции `background` файла `menus.yml`. Можно заменить оба материала, изменить наборы слотов или отключить фон через `enabled: false`. Фон подключается явно только в тех GUI, которым он нужен; функциональные элементы меню имеют приоритет над декоративными слотами.
